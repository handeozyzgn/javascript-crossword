 ...
 * selon le format ".puz" vers ".json".
 * 
 * @author Guy Lapalme, Université de Montréal, 2011, 2015 (pour la création d'un json valide)
 *
 */
public class Puz2json {
     ...
     * @param bis
     */
    Puz2json(BufferedInputStream bis){
        try {
            readHeader(bis);
    ...
    private String field(String name, String val){
        return str(name)+":"+str(val)+",";
    }
    private String field(String name, int val){
        return str(name)+":"+val+",";
    }
    ...
    public String toJSON(){
        ...
        js.append("{");
        js.append(field("nCols",nCols)+field("nRows",nRows)+"\n");
        js.append(field("author",authorName));
        js.append(field("puzzle",puzzleName));
        js.append(field("copyright",copyright)+"\n");
        js.append("\"diagram\":[\n");
        boolean first=true;        
        for(int r=0;r<nRows;r++){
            js.append(first?" ":",");
     ...
     * Écriture de l'information lue du fichier et création d'un fichier .json contenant
     * la version JSON
    ...
    public static void main(String[] args){
        try {
            if(args.length!=1){
                System.out.println("usage: java Puz2json fichier.puz");
                System.exit(1);
            }
            BufferedInputStream bis = new BufferedInputStream(
                                        new FileInputStream(args[0]));
            Puz2json puz = new Puz2json(bis);
            // sortie de la représentation chaîne
            System.out.println(puz);
            // creation de la version json 
            //   nom du fichier est le même que le fichier d'entrée avec json comme extension
            //   au lieu de .puz ou .PUZ en entrée si elle existe
            String jsStream = args[0].replaceAll("(?i)^(.*?)(\\.puz)?$","$1.json");
            PrintStream ps = new PrintStream(jsStream,"UTF-8");
            ps.println(puz.toJSON());
